...
export class AddAccountComponent implements OnInit{
  ...
  constructor(private formBuilder: FormBuilder, private accountService: AccountService, private sharedService: SharedService, private router: Router) {}
  ngOnInit() {
  ...
  onSubmit() {
    if (this.accountForm.valid) {
      ...
      this.accountService.addAccount(formData).subscribe({
        next: (response: any) => {
          this.sharedService.showNotification(true, "Account creation", "Account was created with success!");
        ...
        error: (response: any) => {
          ...
          this.sharedService.showNotification(false, "Account creation", displayMessage);
        }
      });
    }
  }
